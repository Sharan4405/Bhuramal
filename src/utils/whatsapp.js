...
if (!PHONE_NUMBER_ID || !TOKEN) {
  console.warn(
    "WHATSAPP_PHONE_NUMBER_ID or WHATSAPP_TOKEN not set. sendMessage will fail until they are provided.",
  );
}
...
function isLoadTest() {
  return process.env.LOAD_TEST === "true";
}
...
async function makeWhatsAppRequest(body, functionName) {
  try {
    ...
    console.error(
      `Error sending WhatsApp ${functionName}`,
...
export async function markMessageAsRead(messageId) {
  ...
  try {
    const res = await axios.post(API_URL, body, { headers: API_HEADERS });
    console.log(
      `✓ Message ${messageId} marked as read (customer will see blue tick)`,
    );
    return res.data;
  } catch (err) {
    console.error(
...
function getMessageTitle(text) {
  ...
  if (
    ...
    lowerText.includes("proceed to payment")
  )
  ...
  if (lowerText.includes("store address") || lowerText.includes("location"))
    return "Store Location";

  // Cart
  if (lowerText.includes("cart")) return "Cart Information";
  if (lowerText.includes("added") && lowerText.includes("to cart"))
    return "Item Added";

  // Menu and navigation
  if (lowerText.includes("main menu") || lowerText.includes("what can i help"))
    return "Main Menu";
  if (
    lowerText.includes("select a category") ||
    lowerText.includes("place your order")
  )
  ...
  // Support
  if (lowerText.includes("support") && lowerText.includes("queries"))
    return "Support Menu";
...
export async function sendMessage(to, text, saveToDb = true) {
  if (isLoadTest()) {
    console.log(`🧪 Load Test: sendMessage skipped -> ${to}`);
    return;
  }
...
export async function sendButtonMessage(
  ...
  footerText = null,
     ...
     if (isLoadTest()) {
        console.log(`🧪 Load Test: Button skipped -> ${to}`);
        return;
    }
  ...
  const result = await makeWhatsAppRequest(
    buildInteractiveBody(to, interactive),
    "sendButtonMessage",
  );
...
export async function sendListMessage(
  ...
  if (isLoadTest()) {
    console.log(`🧪 Load Test: List skipped -> ${to}`);
    return;
}
...
export async function sendUrlButton(
  ...
  if (isLoadTest()) {
    console.log(`🧪 Load Test: URL Button skipped -> ${to}`);
    return;
}
...
export async function sendLocation(
  ...
  if (result) {
    await saveOutgoingMessage(
      to,
      `📍 Location: ${name || address || "Shared location"}`,
    );
  }
