...
async function sendCartReminder(userId, cart) {
  try {
    ...
    await sendButtonMessage(
      userId,
      `🛒 *apka cart abhi bhi ready hai!*\n\n` +
        `Apne *${totalItems} item(s)* apne cart mein add kiye hain:\n\n` +
        `${itemSummary}${moreItems}\n\n` +
        `💰 Total Amount: ₹${totalAmount.toFixed(2)}\n\n` +
        `Agar order complete karna ho, toh neeche diye gaye button se checkout kar sakte hain. 🌟\n\n` +
        `⏳ *Note:* Aapka cart 24 ghante tak hi save rahega.`,
      [
    ...
    console.error(
      `❌ Failed to send cart reminder to ${userId}:`,
      error.message,
    );
    return false;
  }
...
export async function checkAbandonedCarts() {
  try {
    ...
    console.log(
      `📋 Found ${abandonedCarts.length} abandoned cart(s) to remind`,
    );
    ...
    console.log(
      `✅ Processed ${abandonedCarts.length} abandoned cart reminder(s)`,
    );
  } catch (error) {
...
export async function resetCartReminder(userId) {
  try {
    ...
    console.error(
      `❌ Error resetting cart reminder for ${userId}:`,
      error.message,
    );
  }
}
